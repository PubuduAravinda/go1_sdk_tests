...
DEFAULT_JOINT_POS = np.array([
    0.1,  0.1,  0.1,  0.1,
    0.80, 0.80, 0.80, 0.80,   # ← was 0.82 — training reverted to 0.800!
   -1.5, -1.5, -1.5, -1.5,
], dtype=np.float32)
...
FR_TH_BINDING_MAX = 0.730  # rad — matches training cap (default 0.800 + delta 0.020)
...
# ], dtype=np.float32)
# KD_PER_JOINT = np.array([4.0,4.0,4.0,4.0,  3.5,9.0,3.5,3.5,  5.0,5.0,5.0,5.0])
# KD_PER_JOINT = np.array([4.0,4.0,4.0,4.0,  3.0,8.0,3.0,3.0,  4.0,4.0,4.0,4.0])
#
# TAU_PER_JOINT_ISAAC = np.array([0,0,0,0, 1.2,1.2,1.2,1.2, 0,0,0,0], dtype=np.float32)
KD_PER_JOINT = np.array([4.0,4.0,4.0,4.0,  2.5,8.0,2.5,2.5,  4.0,4.0,4.0,4.0])
TAU_PER_JOINT_ISAAC = np.array([0,0,0,0, 1.5,1.5,1.5,1.5, 0,0,0,0], dtype=np.float32)

# ── Hardware output scaling ───────────────────────────────────────────────────
# FIX 3: HIP_SCALE restored to 0.70 (was 0.5 — too conservative)
HIP_SCALE   = 1.00 #0.70   # ±0.20 × 0.70 = ±0.14 rad max hip hw delta
THIGH_SCALE = 0.85
KNEE_SCALE  = 1.00

# HIP_OBS_CORRECTION = np.array([0.0, 0.0, 0.0, 0.0], dtype=np.float32)
# Zero out HIP_OBS_CORRECTION — jdelta_offset already handles the equilibrium offset
HIP_OBS_CORRECTION = np.array([0.0, 0.0, 0.0, 0.0], dtype=np.float32)
...
MAX_DELTA_PER_JOINT = np.array([
    0.06, 0.06, 0.06, 0.06,
    0.06, 0.06, 0.06, 0.06,   # was 0.08 — halfway to uncalibrated 0.04
    0.08, 0.08, 0.08, 0.08,
], dtype=np.float32)
...
policy = torch.jit.load("go1_deploy_calib_v7/policy.pt").to(device).eval()
print("[POLICY] Loaded go1_deploy_calib_v6/policy.pt (normalise + MLP + tanh baked in)")
with torch.no_grad():
